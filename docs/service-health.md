...
Hover over a latency chart or tap it to select the nearest recorded point by horizontal position. Recent-check tooltips show local date and time, latency, Passed/Failed, and an HTTP status when available. **30 days** tooltips show the UTC date, average, minimum, maximum, and check count. Days without checks remain gaps and cannot be selected.
