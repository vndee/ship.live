...
Totals read all retained, authorized weekly activity without a 20,000-row cutoff. Canonical event IDs deduplicate installation overlap; notes, inbound alerts, and bot activity are excluded. Missing history can lower totals. Personal workspace digests preserve their source selection and mine-only author filter. Immediately before sending, the worker rechecks the repository permissions, saved webhook pins, and source/author scope used to build the digest. A changed scope prevents that payload from being sent.
