...
`server/github-app.ts` handles App OAuth, user/installations/repository lookup, token creation, and bounded recent-history reads. It uses fixed GitHub origins, read-only permissions, explicit timeouts, and complete pagination for access checks. An incomplete or failed permission listing grants no partial cached access. Feed, wall, health, and live-stream reads each recheck access with GitHub, so installation and repository listings send the ETag of the previous identical page. An unchanged page returns `304 Not Modified`, which GitHub does not charge to the user's shared hourly rate limit; the process reuses the parsed page, keyed by a token hash. Changed or revoked access is still seen on the next read.
