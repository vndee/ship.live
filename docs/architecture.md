...
A person connects any number of installations from **Account and GitHub connections** (`PUT /api/github/installations` with the ones to connect and to leave, verified with one read of their GitHub access). An organization's installation makes them a member of its team workspace; their own account's installation attaches to their journal. Leaving removes that membership or attachment only; stored activity stays with the installation. The journal also reads GitHub activity from its sources (`PATCH /api/workspaces/:id/sources`): every connected installation by default, or a chosen subset, and by default only events whose actor is the owner's GitHub login. Its feed reads each source with that installation's synced repository access, rechecks it after the read, and its live stream follows every source. Syncing a journal imports each source.
...
Tokens contain 32 random bytes. PostgreSQL finds a link by its token's SHA-256 hash and keeps a copy of the token encrypted with `TOKEN_ENCRYPTION_KEY`, bound to its workspace and creator, so the creator can copy the active link again. Without the key, or for links created before migration 017, the token is shown only when the link is created, and the UI offers rotation. Links use `/share#token`; the browser sends the token in `x-dashboard-share`, keeping it out of request URLs and referrers. Shared requests omit account cookies, return `no-store`, and expose neither notes nor event bodies. Anyone holding a valid link can see contributor names, GitHub activity titles, repository names, and XP from the pinned scope.

## Engineering utilities wall

Pulse, the team's main view, offers Overview, Review Radar, Release Pulse, Service Health, and Leaderboard scenes as tabs; Overview also lists repositories by this week's activity; choosing one opens its details over the page (`?repository=owner/name`), computed in the browser from the same received events and wall signals, with its full Live feed one click away. Members can set the workspace's Pulse title and subtitle (`PATCH /api/workspaces/:id/pulse`, stored on the workspace and shown on shared links); empty fields fall back to the default. The main navigation holds only Pulse and Service Health: the Live feed, milestones, and contributor table open from Pulse and lead back to it. Review Radar is always offered and Service Health whenever health data is available, each with an empty state; Release Pulse appears once GitHub reports deployments. The chosen scene stays put unless the viewer turns on **Auto-slide**, which starts on for signed-out visitors and the fullscreen wall display while dashboard animation is on, and pauses while the pointer is over the wall. **Arrange tabs** hides and reorders scenes per workspace; the order and visibility are stored only in that browser's localStorage, and at least one tab stays visible. Leaderboard and Team rows open a contributor profile computed in the browser from the same received events and credit rules as the weekly board; it adds no server data. GitHub `check_run`, commit `status`, `workflow_run`, `deployment`, and `deployment_status` webhooks feed a separate current-state store keyed by installation, repository, commit SHA, and signal identity. The app stores bounded names, states, timestamps, and GitHub URLs; it excludes raw payloads, logs, annotations, and source code. Signed webhook updates notify connected wall clients through the existing PostgreSQL-backed SSE path.
...
The Pulse **Delivery** scene measures one deployment environment, production-like names first, over the last 30 days and the 30 days before. Deployments per week counts successful deployments, including those GitHub later marked inactive, at the time ship.live saw them succeed; an inactive deployment whose success it did not see is left out. Change failure rate is failed deployments as a share of finished ones. Time to restore is the median time from a failed deployment to the next successful one from the same repository and environment. Time to merge is the median time from opening a pull request to merging it, across the workspace; it stands in for lead time because GitHub does not link a deployment to the pull requests it ships, and it reads a merged pull request's last update as its merge. Service Health adds the median time to resolve incidents, from the incidents its snapshot lists: up to 20 per service, open or opened or resolved in the last 30 days. When a service reaches that limit, the count reads as a minimum. The figures come from wall signals, bounded to the latest 300 of each kind per repository, so they begin when GitHub started sending deployment webhooks and are cut short when `EVENT_RETENTION_DAYS` is under 60. They describe the team and are never broken down by person.
