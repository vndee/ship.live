CREATE TABLE ship_live_health_shares (
  id uuid PRIMARY KEY,
...
ALTER TABLE ship_live_health_shares ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON ship_live_health_shares FROM PUBLIC;
DO $$
DECLARE role_name text;
BEGIN
  FOREACH role_name IN ARRAY ARRAY['anon','authenticated'] LOOP
    IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname=role_name) THEN
      EXECUTE format('REVOKE ALL ON ship_live_health_shares FROM %I',role_name);
    END IF;
  END LOOP;
END $$;
