...
- Isolate oversized review context per PR, discard broken transaction connections, roll back recap saves when authorization changes, and preserve retryable input errors without exposing unverified data. Deduplicate comparison events before assigning periods and allow local Delivery environment selection.
...
- Replace the Delivery scene's environment menu with a dropdown styled like the rest of the app.
...
- Merge each probe's check strip into its latency chart: in **Recent checks**, failed checks are red and a pass/fail bar under the plot lines up with each check, whose tooltip adds the failure reason. Service-row blocks and days in the 90-day strip show their details on hover or tap.
