...
- **Period comparison.** Overview compares merges, releases, reviews, and participating contributors with the preceding UTC interval of equal length. Open either count to inspect its contributions. Incomplete current periods and limited stored history stay explicit.
- **Review follow-through.** See current PR reasons and age, mark that you are looking, and snooze your own reminder for four hours. Teammates cannot overwrite each other's claim. Changed heads or resolved blockers invalidate old actions; historical and shared views stay read-only.
...
- **Saved views.** Save up to 30 private named views per account, available across browsers. Each preserves its workspace, page, tab, Delivery environment, and supported URL filters. Rolling periods update with the calendar; custom dates stay fixed. Rename, replace with the current view, or delete a saved view. Opening it always checks current workspace access.
...
- **Recap in the app.** Weekly recap shows completed UTC Monday–Sunday weeks, shipped highlights, helpful reviewers, and clearly labeled current PRs needing help. Write a private reflection and export Markdown. Reflections stay private even in team workspaces. Set the workspace digest weekday, local time, and IANA timezone; Monday 09:00 UTC remains the default. This changes when existing digest-enabled webhooks send, not which destinations listen.
