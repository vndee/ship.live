...
- Replicas must share consistent Auth, App, public-origin, and encryption settings. Request limits are counted in PostgreSQL and shared by replicas, falling back to per-process counts while the database is unreachable; concurrent-stream limits are process-local.
- Webhook URLs, header values, and signing secrets are encrypted in PostgreSQL and never returned to the browser after saving. Outbound requests reach only public addresses, using the checked address and no redirects. Inbound webhook URLs carry a random token stored only as a hash, with optional HMAC signatures. Activity reaches a webhook only from repositories its owner pinned and can still see.
- Production responses carry a Content Security Policy without inline scripts, `X-Frame-Options: DENY`, a `Permissions-Policy` that disables unused browser features, and `Cross-Origin-Opener-Policy: same-origin`. `/metrics` stays disabled until `METRICS_TOKEN` is set and reports route patterns and counts, never IDs or tokens. Logs never include request bodies, headers, or query strings.
